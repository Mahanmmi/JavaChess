...
public class ChessBoardUnit extends JButton {
    ...
    public ChessBoardUnit(Coordinate unitCoordinates) {
        this(unitCoordinates, null);
    }

    public ChessBoardUnit(Coordinate unitCoordinates, Piece piece) {
    ...
    private static void updateChecks() {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                ...
                ArrayList<Coordinate> checkeds = thisPiece.getAllMoves(new Coordinate(i, j), chessBoard);
                for (Coordinate checked : checkeds) {
                    int x = checked.getX();
                    int y = checked.getY();
    ...
    private static int colorizeMovables(Coordinate unitCoordinates, Piece thisPiece) {
    ...
    private static void checkWin() {
        int whiteAvaiableMoves = 0;
        int blackAvaiableMoves = 0;
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                Piece thisPiece = chessBoard[i][j].getPiece();
                if (thisPiece == null) {
                ...
                if (thisPiece.isWhite()) {
                    whiteAvaiableMoves += colorizeMovables(new Coordinate(i, j), thisPiece);
                } else {
                    blackAvaiableMoves += colorizeMovables(new Coordinate(i, j), thisPiece);
                }
        ...
        addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseEntered(java.awt.event.MouseEvent evt) {
                if (piece != null && ((piece.isWhite() && turn % 2 == 0) || (!piece.isWhite() && turn % 2 == 1)) && clickedUnit == null) {
                    colorizeMovables(((ChessBoardUnit) evt.getSource()).getUnitCoordinates(), piece);
                }
            }

            public void mouseExited(java.awt.event.MouseEvent evt) {
                if (piece != null && ((piece.isWhite() && turn % 2 == 0) || (!piece.isWhite() && turn % 2 == 1)) && clickedUnit == null) {
                    undoColorizeMovables(((ChessBoardUnit) evt.getSource()).getUnitCoordinates(), piece);
                }
            }
                    ...
                    if (clickedUnit == target) {
                        ...
                        if ((((clickedUnit.piece.isWhite() && turn % 2 == 0) || (!clickedUnit.piece.isWhite()) && turn % 2 == 1))
                            ...
                            if (target.getPiece().getType().equals("Pawn")
                                    && (target.getUnitCoordinates().getY() == 0 || target.getUnitCoordinates().getY() == 7)) {
                            ...
                            //Castling
                            int deltaX = Math.abs(target.getUnitCoordinates().getX() - clickedUnit.getUnitCoordinates().getX());
                            if (target.getPiece().getType().equals("King") && deltaX > 1) {
                                if (deltaX == 2) {
                                    ChessBoardUnit targetPrim = chessBoard[target.getUnitCoordinates().getX() - 1][target.getUnitCoordinates().getY()];
                                    ChessBoardUnit sourceRook = chessBoard[target.getUnitCoordinates().getX() + 1][target.getUnitCoordinates().getY()];
    ...
    public Coordinate getUnitCoordinates() {
